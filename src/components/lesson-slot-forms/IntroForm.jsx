...
export default function IntroForm({ initialHeading, initialData, onSubmit, onCancel }) {
  ...
  function handleSubmit(e) {
    ...
    onSubmit(heading, { paragraphs: cleanParas, didYouKnow: didYouKnow.trim() || undefined })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <input className={INPUT} value={heading} onChange={(e) => setHeading(e.target.value)} placeholder="e.g. Overview" required />
      </div>
      ...
      <div>
        <label className={LABEL}>Did You Know? <span className="normal-case font-normal text-stone-400">(optional)</span></label>
        <textarea className={INPUT + ' min-h-[60px]'} value={didYouKnow} onChange={(e) => setDidYouKnow(e.target.value)} placeholder="A fun or surprising fact about this topic" rows={2} />
      </div>
